...
// // Додай цей лог для перевірки в терміналі:
// const isFirstLogin = await SecureStore.getItemAsync('isFirstLogin');
// console.log("Значення isFirstLogin при вході:", isFirstLogin);

// if (isFirstLogin === 'true') {
//   console.log("Рендеримо Intro...");
...
export default function AuthScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 2. Перевіряємо, чи це перший вхід (мітка ставиться в RegisterScreen)
      const isFirstLogin = await SecureStore.getItemAsync('isFirstLogin');
      
      console.log("=== AUTH DEBUG ===");
      console.log("Token saved successfully");
      console.log("isFirstLogin status:", isFirstLogin);

      if (isFirstLogin === 'true') {
        console.log("Redirecting to: /intro");
        router.replace('/into'); // Переконайся, що файл називається app/intro.tsx
      } else {
