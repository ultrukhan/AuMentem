...
import { View, Text, StyleSheet, ImageBackground, ActivityIndicator, useWindowDimensions, Pressable } from 'react-native';
...
import Animated, { useAnimatedStyle, withTiming, useSharedValue, withRepeat, withSequence, cancelAnimation, useAnimatedReaction } from 'react-native-reanimated';
...
const AmbientOrb = ({ size, top, left, right, bottom, colors, durX, durY, durScale, durOpacity, animationsEnabled }: any) => {
  ...
  useEffect(() => {
    if (animationsEnabled) {
      floatX.value = withRepeat(withSequence(withTiming(15, { duration: durX }), withTiming(-15, { duration: durX })), -1, true);
      floatY.value = withRepeat(withSequence(withTiming(-15, { duration: durY }), withTiming(15, { duration: durY })), -1, true);
      scaleAnim.value = withRepeat(withSequence(withTiming(1.3, { duration: durScale }), withTiming(0.7, { duration: durScale })), -1, true);
      opacityAnim.value = withRepeat(withSequence(withTiming(0.9, { duration: durOpacity }), withTiming(0.1, { duration: durOpacity })), -1, true);
    } else {
      floatX.value = 0; floatY.value = 0; scaleAnim.value = 1; opacityAnim.value = 0.6;
    }
  }, [animationsEnabled]);
  ...
  return (
    <Animated.View style={[{ position: 'absolute', width: size, height: size, top, left, right, bottom, zIndex: 0, borderRadius: size / 2, overflow: 'hidden' }, style]}>
      <LinearGradient colors={colors} style={StyleSheet.absoluteFill} start={{ x: 0.5, y: 0.5 }} end={{ x: 1, y: 1 }} />
    </Animated.View>
  );
};
...
const AnimatedSvgPet = ({ isDark }: { isDark: boolean }) => {
  ...
  return (
    <Animated.View style={[{ position: 'absolute', bottom: 85 + Math.max(insets.bottom, 20), left: 0, zIndex: 10 }, style]}>
      <Pressable onPress={() => { 
        playClickSound(); 
        ...
        {isSleeping && (
          <Text style={{ position: 'absolute', top: -15, right: -10, fontSize: 16, color: c.textMuted }}>
            Zzz
          </Text>
        )}
      </Pressable>
...
export default function HomeScreen() {
  ...
  const maybeShowTracker = useCallback(
    async (token: string) => {
      if (trackerCheckInFlight.current || trackerCheckedThisSession.current) return;
  ...
  return (
    <AnimatedBackground isDark={isDark} themeKey={isDark ? 'dark' : 'light'}>
      ...
      {animationsEnabled && (
        <>
          <AmbientOrb size={45} top="15%" left={30} colors={isDark ? ['rgba(255, 255, 255, 0.25)', 'rgba(255, 255, 255, 0)'] : ['rgba(255, 255, 255, 0.8)', 'rgba(255, 255, 255, 0)']} durX={12000} durY={10000} durScale={6000} durOpacity={5500} animationsEnabled={animationsEnabled} />
          <AmbientOrb size={55} top="35%" right={40} colors={isDark ? ['rgba(255, 255, 255, 0.2)', 'rgba(255, 255, 255, 0)'] : ['rgba(255, 255, 255, 0.7)', 'rgba(255, 255, 255, 0)']} durX={14000} durY={11000} durScale={7500} durOpacity={6500} animationsEnabled={animationsEnabled} />
          <AmbientOrb size={35} bottom="25%" left={60} colors={isDark ? ['rgba(255, 255, 255, 0.3)', 'rgba(255, 255, 255, 0)'] : ['rgba(255, 255, 255, 0.9)', 'rgba(255, 255, 255, 0)']} durX={11000} durY={13000} durScale={5000} durOpacity={7000} animationsEnabled={animationsEnabled} />
          <AmbientOrb size={45} top="50%" left={20} colors={isDark ? ['rgba(255, 255, 255, 0.2)', 'rgba(255, 255, 255, 0)'] : ['rgba(255, 255, 255, 0.8)', 'rgba(255, 255, 255, 0)']} durX={13000} durY={9000} durScale={8000} durOpacity={6000} animationsEnabled={animationsEnabled} />
          <AmbientOrb size={70} bottom="15%" right={50} colors={isDark ? ['rgba(255, 255, 255, 0.15)', 'rgba(255, 255, 255, 0)'] : ['rgba(255, 255, 255, 0.6)', 'rgba(255, 255, 255, 0)']} durX={15000} durY={12000} durScale={7000} durOpacity={8000} animationsEnabled={animationsEnabled} />
          
          {isDark && (
            <>
              <AmbientOrb size={40} top="10%" right={80} colors={['rgba(255, 255, 255, 0.15)', 'rgba(255, 255, 255, 0)']} durX={10000} durY={14000} durScale={6500} durOpacity={5000} animationsEnabled={animationsEnabled} />
              <AmbientOrb size={65} bottom="40%" right={20} colors={['rgba(255, 255, 255, 0.1)', 'rgba(255, 255, 255, 0)']} durX={16000} durY={9000} durScale={5500} durOpacity={7500} animationsEnabled={animationsEnabled} />
              <AmbientOrb size={30} top="70%" left={40} colors={['rgba(255, 255, 255, 0.2)', 'rgba(255, 255, 255, 0)']} durX={12000} durY={15000} durScale={8000} durOpacity={6500} animationsEnabled={animationsEnabled} />
            </>
          )}
