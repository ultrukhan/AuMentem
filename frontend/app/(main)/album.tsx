...
const GAP = 12;
const IMAGE_SIZE =
  (width - Spacing.screenX * 2 - GAP * (COLUMN_COUNT - 1)) / COLUMN_COUNT;
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <Text
              style={[Typography.titleMd, { color: "#FFF", fontSize: 14 }]}
              numberOfLines={1}
            >
              {item.quest_title}
            </Text>
  ...
  return (
    <AnimatedBackground isDark={isDark} themeKey={isDark ? "dark" : "light"}>
      <SafeAreaView style={s.safe} edges={["top", "bottom"]}>
        <Animated.View
          entering={animationsEnabled ? FadeIn.duration(400) : undefined}
          exiting={animationsEnabled ? FadeOut.duration(300) : undefined}
          style={{ flex: 1 }}
        >
          <View style={s.header}>
            <Pressable
            ...
            <View style={s.headerTitleBox}>
              ...
              <Text style={[Typography.nav, { color: c.textMuted }]}>
                Твої досягнення
              </Text>
            </View>
            <View style={{ width: 48 }} />
          ...
          {isLoading ? (
            ...
            <View style={s.centerContainer}>
              ...
              <Text
                style={[
                  Typography.titleMd,
                  { color: c.textMain, marginTop: 16, textAlign: "center" },
                ]}
              >
                Альбом порожній
              </Text>
              ...
              >
                Тут з'являться фотографії, які ти збережеш після виконання
                гео-квестів.
              </Text>
            </View>
          ) : (
        ...
        >
          <View style={s.lightboxOverlay}>
            ...
            {lightboxPhoto && (
              <>
                <Image
                  source={{ uri: lightboxPhoto.photo_url }}
                  style={s.lightboxImage}
                  contentFit="contain"
                  transition={200}
                />
                <View style={s.lightboxCaption}>
                  <Text style={[Typography.titleMd, { color: "#FFF" }]}>
                    {lightboxPhoto.quest_title}
                  </Text>
                  <Text
                    style={[Typography.nav, { color: "#D1D5DB", marginTop: 4 }]}
                  >
                    {lightboxPhoto.location_name} •{" "}
                    {new Date(lightboxPhoto.completed_at).toLocaleDateString(
                      "uk-UA",
