...
import {
  ...
  LocateFixed, MapPin, Camera, Share, Ghost, Check
...
export default function GeoQuestsScreen() {
  ...
  const processQuestCompletion = async (imageUri: string) => {
    ...
    try {
      ...
      formData.append('file', { uri: imageUri, type: 'image/jpeg', name: 'proof_photo.jpg' } as any);
      ...
      if (!cloudData.secure_url) throw new Error('Cloudinary не повернув URL');
...
const s = StyleSheet.create({
  ...
  shareModal: { borderTopLeftRadius: Radii.xl, borderTopRightRadius: Radii.xl, padding: 24, paddingBottom: Platform.OS === 'ios' ? 40 : 24 },
